...
$(document).ready(function () {
    ...
    for(let i of a) {
        if(!i.getAttribute('class').toString().includes("carousel") && !i.getAttribute('class').toString().includes("non")){

            let newUrl = new URL(window.location.origin + i.getAttribute('href').toString());
            if(comParams != null){
                newUrl.searchParams.set('curLang', comParams);
            }
            i.setAttribute('href', newUrl.toString());
        }
    }
